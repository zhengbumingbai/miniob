...
RC BplusTreeHandler::create(const char *file_name,
  ...
  //存储数据的空间减去索引文件头部的空间
  //减去索引节点的大小/（属性长度+两倍的记录ID长度））
  //   为什么×2！！ 因为每一个key是
  //   key_length+RID，而具体的位置会有一个新的RID指到具体的页
  // 所以一个记录需要的空间是key_length + 2 * RID
  file_header->order =
      ((int)BP_PAGE_DATA_SIZE - sizeof(IndexFileHeader) - sizeof(IndexNode)) /
      (file_header->key_length + 2 * sizeof(RID));
...
// zt 修改关键字比较逻辑
// 实际上这个函数有问题pdata指的是左侧目标key pkey指的是拿来比较的key
// 实际调用的时候pdata传入的peky 使用时需要注意
int CmpKey(IndexAttrInfo *attr, int attr_num, const char *pdata,
           const char *pkey, int isUnique = 0) {
  ...
  for (int i = 0; i < attr_num; i++) {
    result =
        CompareKey(key_in_page, target, attr[i].attr_type, attr[i].attr_length);
    if (0 != result) {
      return result;
    }
...
RC BplusTreeHandler::get_entry(const char *pkey, RID *rid) {
  ...
  for (i = 0; i < leaf->key_num; i++) {
    if (CmpKey(file_header_.attrs, file_header_.attr_num, key,
               leaf->keys + (i * file_header_.key_length),
               file_header_.isUnique) == 0) {
...
RC BplusTreeHandler::print_tree() {
  ...
  while (page_num != 0) {
    for (i = 0; i < node->key_num; i++) {
      ...
      printf(
          "key : %d, rids in key (page_num:%d slotnum %d), rids (page_num:%d "
          "slotnum %d)\n",
          *(int *)pkey,
          (*(RID *)(pkey + file_header_.key_length - sizeof(RID))).page_num,
          (*(RID *)(pkey + file_header_.key_length - sizeof(RID))).slot_num,
  ...
  for (size_t i = 0; i < compare_objects_.size(); i++) {
    ...
    if (value_ == nullptr) {
      if (comp_op_ == CompOp::IS) {
        int8_t is_null = *(int8_t *)(pkey + offset + attr_length - 1);
        if (is_null) {
          flag = true;
        } else {
          flag = false;
