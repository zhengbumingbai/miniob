...
RC TableMeta::init(const char *name, int field_num,
  ...
  for (int i = 0; i < field_num; i++) {
    ...
    rc = fields_[i + sys_fields_.size()].init(attr_info.name, attr_info.type, field_offset, attr_info.length, true, bool(attr_info.nullable));
    if (rc != RC::SUCCESS) {
...
std::vector<FieldMeta> &TableMeta::fields() { return fields_; }
...
const IndexMeta *TableMeta::find_index_by_field(
    std::vector<std::string> atteibute_names) const {
  for (const IndexMeta &index : indexes_) {
    // zt
    // 逻辑是如果fields的所有属性名都满足该索引的前n个属性名即可匹配，索引的关联列数应该大于等于属性个数
    bool isMatch = 1;
    for (int i = 0; i < atteibute_names.size(); i++) {
      if (index.field_names()[i] != atteibute_names[i]) {
        isMatch = 0;
        break;
      }
    }
